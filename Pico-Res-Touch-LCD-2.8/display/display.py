...
class Textbox:
    def __init__(self, string = '', scroll_speed = 0, orientation = 0): #initial configurations. Can be changed with below functions.
        ...
        self.font = arial10
        self.wri = CWriter(self.ssd, self.font, verbose=False)
    ...
    def setup(self): #Legacy function meant to setup the textbox
        refresh(self.ssd, True)  # Initialise and clear display.
        CWriter.set_textpos(self.ssd, 0, 0)  # In case previous tests have altered it
        self.wri = CWriter(self.ssd, self.font, verbose=False)
        self.wri.set_clip(True, True, False)
    ...
    def write(self, s = ""): #For compatibility with print
        s = str(s)
        self.s += "\n"+s
        self.update()
        return s
    
    def read(self, s = ""): #For compatibility with print
        s = str(s)
        return s
...
class Screen():
    def __init__(self, ssd = ssd, orientation = 0): #0 for portrait, 1 for mirror mode portrait, 2 for down portrait, 3 for down portrait mirror mode, etc.
        gc.collect() 
    ...
    def fillCircle(self, x = 0, y = 0, radius = 7, color = GREEN): #Draws a filled circle with origin at the diagonal of a box
        self.fillcircle(self.lcd_display, x+radius, y+radius, radius, color)
    ...
    def defineGUI(self, nrow = 2, ncol = 2, width = 128, nlines = 7, font = arial10): #Defines all proceeding GUI functions
        self.posArg = (nrow, ncol, width, nlines)
        self.wri = CWriter(self.self.ssd, font, verbose=False)
        self.wri.set_clip(True, True, False)
