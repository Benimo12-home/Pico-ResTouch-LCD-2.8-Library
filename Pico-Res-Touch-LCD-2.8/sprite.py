...
class Cell:
    def __init__(self, dev, path, xorigin, yorigin, palette, size, character):
        self.dev = dev
        self.x = (xorigin*size) + Scroll(x,0)
        self.y = (yorigin*size) + Scroll(0,y)
        self.palette = palette
        self.size = size * 8 #Size here refers to the size of the cell i.e size 1 means it is an 8 * 8 cell, size 2 means it is 16 * 16, etc.
        self.character = character #This should work similarly to a sprite.
        c = CellMap(dev, path, size, 1, 1)
    
    def drawCell():
        c.drawCharacter(self.character, self.palette, self.x, self.y)
        
    
class CellMap:
    def __init__(self, dev, path, size, xlimit = 50, ylimit = 50):
        self.dev = dev
        self.path = path
        self.size = size
        self.xlimit = xlimit
        self.ylimit = ylimit
        self.cellmap = sprites.readCellmap
        
    #def getCharacter(self, character):
    
    def drawCharacter(self, character, palette, x = 0, y = 0):
        x = x + self.x # X modification to account for scrolling
        y = y + self.y # Y modification to account for scrolling
        x_index = x-x #creates an index for a matrix loop
        y_index = y-y #creates an index for a matrix loop
        pixel = 0 #Counter for the function. It also tells the Pico which pixel is to be filled
        colorkeys = {v: k for k, v in palette.items()}
        for y_index in range(8*self.size): #For the y_coordinate
            for x_index in range(8*self.size): #For the x-coordinate
                color = palette.get(self.getCharacter(character)[pixel]) #Fetches the specified color from the palette
                colorkey = colorkeys.get(self.getCharacter(character)[pixel]) #Fetches the specified color from the palette
                if color != 0: #Make color 0 transparent
                    if isinstance(color, int) == True:
